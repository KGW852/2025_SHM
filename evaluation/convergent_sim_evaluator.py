...
from evaluation.post_eval import AnomalyDetector
...
class ConvergentSimEvaluator:
    ...
    def __init__(self, cfg: dict, mlflow_logger: MLFlowLogger, run_id: str, last_epoch: int, device: torch.device = None):
        ...
        self.anomaly_detector = AnomalyDetector(self.cfg)

    def test_epoch(self, data_loader, epoch):
        """
        Load the checkpoint corresponding to the epoch to update the weights of self.encoder
        Collect two types of labels (class_label, anomaly_label)
        Return anomaly metric results based on self.method (simsiam / distribution)
        """
        ...
        pbar = tqdm(data_loader, desc=f"[Test]  [Epoch {epoch}/{self.last_epoch}] {self.method}", leave=False)
        with torch.no_grad():
            for (x_s, lbl_s), (x_t, lbl_t) in pbar:  # lbl_s, lbl_t: tensor([class_label, anomaly_label])
                class_lbl_s = lbl_s[:,0]  # (B,)
                anomaly_lbl_s = lbl_s[:,1]
                class_lbl_t = lbl_t[:,0]
                anomaly_lbl_t = lbl_t[:,1]
                ...
                all_class_labels.append(class_lbl_s.cpu())
                all_anomaly_labels.append(anomaly_lbl_s.cpu())
        ...
        if self.method == "simsiam":
            metrics_dict = self.anomaly_detector.evaluate_simsiam(p1=all_p_s, z2=all_z_t, p2=all_p_t, z1=all_z_s, anomaly_labels=all_anomaly_labels)

        elif self.method == 'distribution':
            metrics_dict = self.anomaly_detector.evaluate_distribution(feats=all_z_s, labels=all_anomaly_labels)

        else:
            raise ValueError(f"[test_epoch] Unknown method: {self.method}")

        print(f"[Test]  [Epoch {epoch}/{self.last_epoch}] {self.method}: {metrics_dict}")
        return metrics_dict
