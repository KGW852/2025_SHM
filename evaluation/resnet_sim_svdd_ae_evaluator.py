...
class ResSimSVDDAEEvaluator:
    ...
    def run(self, eval_loader, test_loader):
        ...
        for epoch in test_run_epochs:
            ...
            enc_umap_path = f"{save_dir}/umap/umap_s1(23)_encoder_epoch{epoch}.png"

            os.makedirs(f"{save_dir}/pca", exist_ok=True)
            enc_pca_path = f"{save_dir}/pca/pca_s1(23)_encoder_epoch{epoch}.png"
            ...
            anomaly_data_path = f"{save_dir}/metric/anomaly_s1(23)_scores_epoch{epoch}_{self.method}.csv"
            anomaly_metric_path = f"{save_dir}/metric/anomaly_s1(23)_metric_epoch{epoch}_{self.method}.csv"

            # Histogram plot
            hist_scores = [res["distance"] for res in test_results]

            os.makedirs(f"{save_dir}/histogram", exist_ok=True)
            hist_path = f"{save_dir}/histogram/anomaly_s1(23)_hist_epoch{epoch}_{self.method}.png"

            # Extract all scores
            os.makedirs(f"{save_dir}/scores", exist_ok=True)
            recon_scores_path = f"{save_dir}/scores/s1(23)_scores_epoch{epoch}_recon.csv"
            distance_scores_path = f"{save_dir}/scores/s1(23)_scores_epoch{epoch}_distance.csv"
